document.addEventListener('DOMContentLoaded', () => {
    ...
    const fetchHomePage = async (page = 1, scrollTop = true) => {
        ...
        try {
            ...
            log('Home fetch ok:', {
                ...
                unknownTotal: homeState.unknownTotal
            });

            if (scrollTop) {
                const rect = matchesTable.getBoundingClientRect();
                if (rect.top < 0 || rect.bottom > window.innerHeight) {
                    matchesTable.scrollIntoView({
                        behavior: 'smooth',
                        block: 'start'
                    });
                }
    ...
    if (matchesTable && pager && !pager.dataset.bound) {
        ...
        if (form) {
            form.addEventListener('change', () => {
                log('Home filters change -> page 1');
                fetchHomePage(1, true);
            });
            form.addEventListener('submit', (e) => {
                e.preventDefault();
                log('Home filters submit -> page 1');
                fetchHomePage(1, true);
                homeFilters?.setAttribute('aria-hidden', 'true');
            });
        }
    ...
    const fetchUserPage = async (page = 1, scrollTop = true) => {
        ...
        try {
            ...
            log('User fetch ok:', {
                ...
                items: parsed.items.length,
                unknownTotal: userState.unknownTotal
            });

            if (scrollTop) {
                const rect = userTable.getBoundingClientRect();
                if (rect.top < 0 || rect.bottom > window.innerHeight) {
                    userTable.scrollIntoView({
                        behavior: 'smooth',
                        block: 'start'
                    });
                }
    ...
    if (userTable && userPager && !userPager.dataset.bound) {
        ...
        if (form) {
            form.addEventListener('change', () => {
                if (userState.userId) {
                    log('User filters change -> page 1');
    ...
    const enhanceSearch = (input) => {
        ...
        const fetchSuggest = async (q) => {
            ...
            try {
                const resp = await fetch(`/db/search/suggest?q=${encodeURIComponent(q)}`, {
        ...
        input.addEventListener('keydown', (e) => {
            ...
            if (e.key === 'ArrowDown') {
                ...
                if (idx >= 0 && idx < entries.length) {
                    e.preventDefault();
                    window.location.href = entries[idx].url;
                }
